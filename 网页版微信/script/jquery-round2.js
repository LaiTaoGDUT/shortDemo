...
"use strict";
	  ...
	  $.post("http://47.106.74.100/login",{
		...
		if($result === "failed"){
		  ...
		  setTimeout(function() {
			...
			$(".already_landed").show();
			$(".edit_area").val("");
			$.get("http://47.106.74.100/getUserInfor",{
			  id: $userid
			},function(data){
  ...
  function GetFriendsList() {                                           //获取好友列表
	  $.post("http://47.106.74.100/getList",function(data){
  ...
  $(".exit").click(function() {
    $.get("http://47.106.74.100/logout",function(data){
      var $result = data.result;
      if($result === "success"){
		$(".hide_two").fadeOut();
        SlidePoint("登出成功！");
  ...
  $(".real_search_list").children("div").on("click",".search_item",function() {              //点击搜索结果跳到聊天框
	...
	if($searchId != $titleId) {
	  ...
	  if($editArea.val().replace(/\s+/g,"") != "") {
		$("#" + $titleId).find("pre").text($editArea.val());
		$("#" + $titleId).find(".draft").show();
	  }else {
		if($(".real_content").children("div").children("div").length == 1) {
		  $("#" + $titleId).find(".draft").hide();
		  $("#" + $titleId).find("pre").text("");
		}else {
		  $("#" + $titleId).find(".draft").hide();
		  $("#" + $titleId).find("pre").text($(".real_content").children().children("div:last").find("pre").text());
		}
	  }
	  $editArea.val("");
	}
	...
	GetChatRecord();
	$(".recent_chat_list").prepend($("#" + $titleId).detach());                //聊天跳到顶部
  ...
  $(".action_area").children("a").on("click",function() {                         //点击发消息快速定位到指定聊天
	...
	GetChatRecord();
	$(".recent_chat_list").prepend($("#" + $titleId).detach());
	$(".your_value").text($titleId);
	$(".your_details").children("img").attr("src",$titleImg);
	$(".nick_area").children().text($titleName);
  });
  ...
  function CreateRecentList(data) {                                         //创建最近聊天框
	...
	if($("#" + data.sender).length <= 0) {
	  var $src = $("." + data.sender).children("img");
	  ...
	  $icon.text(sum);
	  $infoNickName.text($nickName.text());
  ...
  function GetRecentList() {                                            //获取未读消息
	$.get("http://47.106.74.100/getUnreadChatRecord",{
  ...
  $(".recent_chat_list").on("click",".recent_item",function(){             //点击最近聊天好友生成聊天框
	...
	if(!$(this).hasClass("highlight")) {
	  ...
	  $(".title_name").text($titleName);
	  $editArea.val("");
	  if(!$("#" + $titleId).find(".draft").is(":hidden")) {
		$editArea.val($("#" + $titleId).find("pre").text());
		$("#" + $titleId).find(".draft").hide();
	  }
	  GetChatRecord();
	  $(".your_value").text($titleId);
  ...
  function GetChatRecord() {                              //获取与好友的聊天记录
	$.get("http://47.106.74.100/getChatRecord",{
      id: $titleId
	},function(data) {
	  var length = data.length;
	  var i;
	  ...
	  $("#" + $titleId).find("pre").text($(".real_content").children().children("div:last").find("pre").text());
	  $("#" + $titleId).children(".ext").children().text($(".real_content").children().children("div:last").find("span").text());
	});
  }
  ...
  $(".btn_send").click(function(){                                              //发送信息
	...
	if($titleId === ""){
      ...
      $.post("http://47.106.74.100/SendContent",{
	    receiver: $titleId,

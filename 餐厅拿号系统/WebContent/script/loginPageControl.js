        ...
        if(loginInformationValidation(userId, userPassword)) {
            $.post("/restaurant_queue_system/loginServlet", {
                ...
                if(result.result == "success") {
                    ...
                    switchLoginToMain();
                    createAllOrder();
                    updateOrder = setInterval(updateOrderMsg,2000);
                    acquireRestaurantInfo();
                } else {
                	alert(result.reason);
                }
            })
...
function acquireRestaurantInfo() {
    $.post("/restaurant_queue_system/getRestaurantInfoServlet", {
...
function updateOrderMsg() {
    ...
    $.post("/restaurant_queue_system/getCurrentOrderServlet", {
        //no parameter
    }, function(result) {
        if(result.result == "failed") {
...
function logout() {
	...
	logoutBtn.on("click", function() {
		$.post("/restaurant_queue_system/logoutServlet", {
				//no parameter
		}, function(result) {
			if(result.result == "failed") {
				alert(result.reason)
